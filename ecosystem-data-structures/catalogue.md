...
<!-- Detailed data-structure catalogue retained for progressive disclosure. -->
...
**Cross-references**: [DS-002], [MEM-COPY-016] (the Wall-1 law + converged equilibrium), [MEM-SAFE-027] (the Wall-2 cross-package caveat), [MOD-LAYER].
...
the Institute swift-linter rules and the package-graph analysis owner. The
single-file carrier-bound slice belongs to SwiftSyntax; cross-file and
cross-package resolution belongs to the typed package analysis.
...
[DS-025] form and are never visited. The remaining predicate surface belongs to
the Swift package-analysis owner: the inherited
...
[API-IMPL-023] track). [VERIFICATION: SwiftSyntax + typed package graph]
...
1. **Canonical-in-main** *(unchanged)*, with a target-placement rider: `swift-X-primitives`
   ...
   ([MOD-TARGET]; the array manifest already scaffolds the "Small type/ops/variant" MARKs),
   exposed through a deliberate aggregate per [MOD-PRODUCT] — so heap-only consumers keep a lean dependency graph
...
**Cross-references**: [DS-002], [DS-005], [DS-025], [DS-028], [DS-029]; **swift-package** [PKG-NAME-*]; **modularization** [MOD-PRODUCT], [MOD-TARGET].
...
**Cross-references**: [DS-025], [DS-026], [DS-028]; **memory-safety** [MEM-SAFE-028]; **modularization** [MOD-LAYER], [MOD-BOUNDARY].
...
- **reuse-first** skill for selecting and composing the ecosystem's owned capabilities
