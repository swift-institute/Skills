...
- **Advisory `continue-on-error` legs** during a matrix transition — e.g., `linux-nightly` per [CI-021] and the `apple-simulator-build` advisory. These remain matrix entries (the doctrine holds); they're temporarily non-gating per documented soak protocols.
- ⚠️ **`embedded-wasm-sdk`, `android-build` and `static-linux-musl-build` are L1-only, NOT universal-matrix legs** (corrected 2026-07-25; this text previously listed all four as peer advisories of the universal matrix, which the mechanism does not do). Measured: the universal reusable defines `apple-simulator-build` and does **not** define the other three; all three are defined in the L1 layer wrapper. **Consequence: L2 and L3 packages are never exercised on Wasm, Android or static-Linux-musl.** Of the four jobs the L1 wrapper adds, only `embedded` carries a stated layer rationale — freestanding-buildability per [PRIM-FOUND-001], recorded in that wrapper's own header. **For the other three the reason for L1-only scoping is unrecorded**; they are described here as observed placement, not as a design decision, and no rationale should be inferred from this entry. Open decision: promote them to the universal (all layers gain the coverage, at the cost of surfacing new failures across two layers at once) or keep them L1-only and write down why.
- **Platform-identity declarations per [CI-114]** (amendment 2026-07-14) — a package whose specification or identity excludes a platform declares `platform-support` in its thin caller and the excluded legs skip. This is NOT matrix derivation (nothing is read from `Package.swift`), NOT cost-based collapse ([CI-096] unchanged), and default-inert (a non-declaring package gets the full matrix). Provenance: principal ruling 2026-07-14, an internal decisions record.
