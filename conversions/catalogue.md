...
description: |
  ...
  Apply when working with typed Index values, Offset, Count, Ordinal, Cardinal,
  cross-domain index conversion, or accessing rawValue.
---

<!-- Detailed conversion rule catalogue retained for progressive disclosure. -->
...
**Statement**: `Index<Element>` is a typealias for `Tagged<Element, Ordinal>`. The phantom `Element` is bound `~Copyable & ~Escapable` per [API-NAME-010b] (maximal suppression on phantom parameters) — it is never stored, so both capabilities are suppressed.
...
**ADT Tower rider — tower op-body index hygiene (M6, ratified 2026-07-03; transcribed at W4, 2026-07-06).** In a tower operation body over the typed seam count (`Index<Element>.Count`), bounds MUST be derived through the typed API — `count.map { Ordinal() }` / typed `Index`/`Offset`/`Count` arithmetic (tiers 2–3) — descending to raw `Int` ONLY via `Int(clamping:)` (tier 4) for the residual arithmetic seed a raw loop genuinely needs. FORBIDDEN in tower op bodies: the reach-through `Int(x.underlying.rawValue)` (a tier-5 double-unwrap) and `Int(bitPattern:)`-in-arithmetic. Both are AST-lint-promotion candidates (extending `no_int_bitpattern_arithmetic`, [CONV-010]). Provenance: `Research/adt-tower.md` §4.7, §2 D4.3.
...
**Lint enforcement**: SwiftLint custom rule `no_int_bitpattern_arithmetic` (`swift-institute/.github/.swiftlint.yml`) catches `Int(bitPattern: x.y)` followed by `+`/`-`/`*`/`/`. Excludes `*Standard_Library_Integration` targets where `Int(bitPattern:)` legitimately lives per [IMPL-010]. Added Wave 2b 2026-05-10.

**Cross-references**: [IMPL-010]
...
**Enforcement (partial)**: Mechanical — `Lint.Rule.RawValue.TaggedUnchecked` (`tagged unchecked with typed alternative`, primitives tier, Bundle.primitives; promoted 2026-07-07 from swift-tagged-primitives' nested `Lint/` PoC per principal ruling). Covers the tier-5 slice only: `Tagged<…>(_unchecked:)` call sites, with `map`/`retag`/`@Test` structural exemptions. Tiers 1–4 preference ordering remains text-only judgment. Receipt: `swift-primitives-linter-rules/Research/promote-tagged-unchecked-validation-2026-07-07.md`.
...
**Cross-references**: [CONV-016], [CONV-017], [CONV-018], [CONV-007], [CONV-010]
...
### [IDX-019] Integer Literals for Compile-Time-Constant Index / Tagged Values

**Statement**: Compile-time-**constant** `Index<T>` / `Tagged` values MUST be written as bare integer literals (the tagged-primitives SLI carve-out makes them infer); **runtime** ints are not literals and keep the explicit `Index<Element>(Ordinal(UInt(slot)))` construction.

**Correct**:
```swift
slab[0] = x                                  // constant index — SLI literal
let i = Index<Element>(Ordinal(UInt(slot)))  // runtime int — explicit construction
```
...
**Cross-references**: [TEST-018], [TEST-010] (target declaration for Test Support), [MOD-PRODUCT]
...
See also:
- **implementation** skill for [PATTERN-017] rawValue access location, [PATTERN-019] no blanket Tagged init (the typed-arithmetic-over-`__unchecked` preference is this skill's [CONV-010])
- **implementation** skill's [IMPL-011] for typed slot/region access primitives and `Lint.Rule.Memory.PointerArithmetic` (covers `Pointer<T>` subscripts with `Index<T>`)
