...
<!-- Detailed experiment rule catalogue retained for progressive disclosure. -->
...
Forbidden layer-level `_index.json` examples are `swift-primitives/Experiments/_index.json`, `swift-standards/Experiments/_index.json`, and `swift-foundations/Experiments/_index.json`; their parent
...
Capture each command's complete output in the experiment record. Output files
are working artifacts, not committed by default. The `main.swift` header is the
...
1. Add a `release-mode-pass.txt` receipt file capturing `workspace package build --fresh --argument=-c --argument release` output.
