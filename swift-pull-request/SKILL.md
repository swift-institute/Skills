...
### [SWIFT-PR-012] PR Body Claim Audit + HEREDOC Body-Escaping

**Statement**: Every claim in a PR body MUST cite a specific file:line, commit SHA, or empirical observation. Temporal markers ("predates", "no longer", "always since"), causal markers ("because", "due to"), and quantifier markers ("always", "never", "every") require explicit evidence; default pattern is direct-cite-source-line. Pure-deletion PRs MAY substitute *enumerative non-regression evidence* (grep across the test suite for the deletion's reachability conditions) for sample-of-one local test runs — strictly stronger and cheaper. HEREDOC body-escaping (`gh pr create --body "$(cat <<'EOF' ... EOF)"`) under a single-quoted heredoc means backticks/dollar-signs are literal; backslash-escaping them produces literal `` \`...\` `` in the output. Use unescaped backticks as the canonical form.

**Composite:** claim-audit rule (semantic) + enumerative-non-regression pattern (mechanical) + HEREDOC escaping note (mechanical).

**Claim categories that MUST cite evidence**:

| Category | Marker examples | Required evidence |
|----------|----------------|-------------------|
| Temporal | "predates", "no longer", "always since" | Git log / commit SHA / file mtime |
| Causal | "because", "due to" | Source line citation showing the mechanism |
| Quantifier | "always", "never", "every", "all" | Enumeration command + output |
| Reachability | "this path is unreachable", "no caller hits this" | grep across affected scope; enumeration |

**Enumerative non-regression evidence pattern** (for pure-deletion PRs):

```bash
# Example: deletion's reachability is bounded by feature flag InternalImportsByDefault
grep -rn "InternalImportsByDefault" Sources Tests | grep -i "<affected-path-pattern>"
# If empty: the deletion is non-regressing by construction.
# Strictly stronger than a sample-of-one local test run.
```

**HEREDOC escape pattern**:

```bash
# Right (single-quoted heredoc, unescaped backticks):
gh pr create --body "$(cat <<'EOF'
The change to `MoveOnlyChecker.cpp:842` removes a stale assertion.
EOF
)"

# Wrong (escaped backticks produce literal \`...\` in output):
gh pr create --body "$(cat <<'EOF'
The change to \`MoveOnlyChecker.cpp:842\` removes a stale assertion.
EOF
)"
...
| Defect | Origin | Fix |
|--------|--------|-----|
| "predates IIBD (SE-0409)" temporal marker | Comment-and-PR-prep v1; assertion was added Feb 2024 (SE-0409 accepted late 2023; the temporal claim was wrong-by-direction) | Removed temporal marker; cited file:line evidence instead |
| "mirroring sibling assertion" padding | Same | Six-round adversarial discussion stripped; replaced with file:line citation |
| Reachability/failure conflation in PR body | Same | Separated "reachable only on this path" (architecture claim) from "no test triggers this" (enumerative claim) |
| Backslash-escaped backticks in PR body | Pre-commit verification on PR shipping | Caught at converged-plan rewrite before public PR; canonical form: unescaped backticks |

**Rationale**: PRs to upstream projects are read by maintainers who do not have the author's session context. Unsupported temporal/causal/quantifier claims either (a) require the maintainer to do the work the author should have done, or (b) get accepted with the author's confidence carrying the day — the latter masks defective evidence chains. Direct-cite is the default; enumerative is the upgrade for purely-mechanical claims.


**Cross-references**: [SWIFT-PR-006] (PR Body)
...
**Issue links**: Include `Resolves https://github.com/swiftlang/swift/issues/NNNNN` in the body or PR. `Resolves` may cite an EXISTING upstream issue after confirming it describes the same bug; when no matching upstream issue exists, do NOT file one — omit `Resolves` and reference the swift-institute/Issues dossier in the PR body instead (per the never-file-upstream rule; see issue-investigation).
