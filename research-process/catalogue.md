...
<!-- Detailed research rule catalogue retained for progressive disclosure. -->
...
Forbidden layer-level `_index.json` examples are `swift-primitives/Research/_index.json`, `swift-standards/Research/_index.json`, and `swift-foundations/Research/_index.json`; their parent
...
Convention sources: **code-surface** [API-NAME-*], [API-ERR-*], and
[API-IMPL-*], plus **implementation** [IMPL-*].
...
The classification is structural: (a) genuinely cross-cutting → the shape is domain-agnostic across unrelated domains; (b) domain-owned vocabulary → governed by `[MOD-OWNER]` alone; (c) layer-agnostic primitive surfaced higher up → pulled down to L1 by default, with the originating L2/L3 package as the first consumer.
...
3. If case (b): cite `[MOD-OWNER]`; demonstrate the type belongs to the domain's coherent semantic vocabulary. No further hurdle.
4. If case (c): identify the correct L1 home per `[MOD-OWNER]` (existing primitives package or, if none fits, a new domain package whose scope is justified independently). The originating L2/L3 package becomes the first consumer; no second-consumer evidence required. Document the pull-down decision so the architectural intent is visible.
...
**Cross-references**: [RES-005], [RES-020], [RES-020a], [MOD-OWNER], [MOD-PACKAGE], [ARCH-LAYER-001], [ARCH-LAYER-008], [API-NAME-003]
...
**Cross-references**: [RES-013a], [EXP-003], [REUSE-006]
...
| Minimal package via `workspace package build --argument=-c --argument=release` | (above factors come back) | Source code, Swift version, optimization level, package shape |
